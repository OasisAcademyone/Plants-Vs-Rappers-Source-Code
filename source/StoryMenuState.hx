...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var BG:FlxSprite = new FlxSprite().loadGraphic(Paths.image('StoryMenuBG'));
		BG.screenCenter();
		add(BG);
		...
		scoreText.setFormat(Paths.font("Cafeteria-Bold.otf"), 32, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreText.borderSize = 2;

		txtWeekTitle = new FlxText(5, 7, 0, "", 32);
		txtWeekTitle.setFormat("HouseofTerror-Regular.ttf", 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		rankText.setFormat(Paths.font("HouseofTerror-Regular.ttf"), 32);
		...
		zombiehand = new FlxSprite(50,720).loadGraphic(Paths.image('HandBurst'));
		...
		clouds = new FlxSprite(2000, -280).loadGraphic(Paths.image('PurpleCloudsFront'));
		clouds.screenCenter();

		clouds1 = new FlxSprite(700, -280).loadGraphic(Paths.image('PurpleCloudsFront'));
		clouds.screenCenter();
		...
		FlxTween.tween(clouds, {x: -5088}, 40, {type: PINGPONG, ease: FlxEase.quadOut});

		FlxTween.tween(clouds1, {x: -2088}, 20, {type: PINGPONG, ease: FlxEase.quadInOut});

		var disclaimer:FlxText = new FlxText("UNDER CONSTRUCTION");
		disclaimer.setFormat(Paths.font("Cafeteria-Bold.otf"), 25, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		disclaimer.y = 670;
		disclaimer.x = 555;
		disclaimer.borderSize = 2.4;
		add(disclaimer);
	...
	function selectWeek()
	{
		if (!weekIsLocked(curWeek))
		{
			...
			FlxG.sound.playMusic(Paths.music('InsaneLaugh'));

			FlxG.sound.play(Paths.sound('HandRise'));

			new FlxTimer().start(0.06, function(tmr:FlxTimer)
			{
				FlxTween.tween(Black, {alpha: 0.7}, 0.43, {ease: FlxEase.quadOut});

				FlxTween.tween(zombiehand, {y: 50}, 0.43, {
					ease: FlxEase.quadInOut,
					onComplete: function(twn:FlxTween)
					{
						FlxTween.tween(zombiehand, {y: 80}, 0.43, {ease: FlxEase.quadInOut});
					}
				});
			});
