...
class OptionsState extends MusicBeatState
{
	...
	override function create() {
		...
		bg = new FlxSprite().loadGraphic(Paths.image('AlmanacLayer'));
...
class PreferencesSubstate extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		descText = new FlxText(50, 550, 1180, "", 32);
		descText.setFormat(Paths.font("Cafeteria-Bold.otf"), 40, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override function update(elapsed:Float)
	{
		...
		if(usesCheckbox) {
			...
			if(controls.UI_LEFT || controls.UI_RIGHT) {
				...
				if(holdTime <= 0) FlxG.sound.play(Paths.sound('selection'));
				holdTime += elapsed;
			} else {
				holdTime = 0;
			}
	...
	function changeSelection(change:Int = 0)
	{
		...
		if(options[curSelected] == 'Anti-Aliasing') {
			if(showCharacter == null) {
				showCharacter = new Character(840, 170, 'zombiebf', true);
